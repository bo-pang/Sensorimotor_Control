...
load("Sensorimotor_mimic_DF.mat");
% G{1} = G{1}/tau;
% G{2} = G{2}/tau;
...
for j=1:5
    ...
    for i=1:N-1
        ...
        dx = (A*x(:,end)+B*u)*dt +...
            B*(G{1}*u*rv1(1,i)+G{2}*u*rv1(2,i))*sqrt(dt);
        xnext = x(:,end)+dx;
        if abs(xnext(1))>0.025 || xnext(2)>0.01 || norm(xnext(1:2))<0.002
            break;
        else
            x = [x, xnext];
        end
    ...
    figure(fig1);
    subplot(1,4,2);
    line([-0.025,-0.025],[-0.2,0.01],'color','k','LineWidth',1);hold on;
    line([0.025,0.025],[-0.2,0.01],'color','k','LineWidth',1);hold on;
    box;
    plot(0, 0, '.r', 'MarkerSize',50); hold on;
    axis([-0.05 0.05 -0.3 0.05]);
    ...
    subplot(4,4,6);
    plot(t(1:i),x(4,1:i),'b');hold on;
    xlabel('time (s)');
    xlim([0,0.6]);
    ylim([-0.5,2]);
    
    subplot(4,4,10);
    plot(t(1:i),x(5,1:i),'b');hold on;
    xlabel('time (s)');
    xlim([0,0.6]);
    ylim([-10,10]);
    
    subplot(4,4,14);
    plot(t(1:i),x(6,1:i),'b');hold on;
    xlabel('time (s)');
    xlim([0,0.6]);
